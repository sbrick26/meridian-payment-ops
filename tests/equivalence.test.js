...
/* verbatim from server.js:676-683 */
function statusDescription(s) {
	if (s === 'PENDING')   return 'Awaiting first review';
	if (s === 'REVIEW')    return 'Under review by an AP clerk';
	if (s === 'HOLD')      return 'Held - awaiting vendor or goods receipt';
	if (s === 'ESCALATED') return 'Escalated to AP controls';
	if (s === 'RESOLVED')  return 'Closed - released or returned';
	return 'Unknown';
}

/* scoreRow — verbatim copy of server.js scoring logic */
