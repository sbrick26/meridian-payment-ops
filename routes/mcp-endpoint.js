 ...
 *
 * Actual v2 endpoints in this service:
 *   GET /api/v2/payment-status?ref=<ref>
 *   GET /api/v2/payment-status?invoice=<invoice>
 *   GET /api/v2/risk-score?ref=<ref>
 *
 * Write paths (/api/v2/payment-status/release, /api/v2/payment-status/hold) do
 * not exist in Phase 1. The tools are declared so the identity refusal can be
 * demonstrated; the ops scope check fires before the upstream call is ever made.
 */
...
const TOOLS = [
  {
    ...
    // Maps to GET /api/v2/payment-status?ref=<ref> or ?invoice=<invoice>
    call: (a) =>
      a.ref
  ...
  },
  {
    ...
    // Maps to GET /api/v2/payment-status with status/q query params.
    // The payment-status route accepts ref or invoice for single-record lookups;
    // for search, status and q are the supported filter params.
    call: (a) => ({ method: 'GET', path: `/api/v2/payment-status${query(a)}` }),
  },
  {
    name: 'payment_risk',
    scope: 'inquiry',
    ...
    // Maps to GET /api/v2/risk-score?ref=<ref>
  ...
  },
  {
    ...
    // Phase 1: this path does not exist. The ops scope check fires before the
    // upstream call — the path is intentionally unreachable at the identity layer.
    call: (a) => ({
      method: 'POST',
      path: `/api/v2/payment-status/release`,
      body: { ref: a.ref, note: a.note || null },
    }),
  },
  {
    name: 'payment_hold',
    ...
    call: (a) => ({
      method: 'POST',
      path: `/api/v2/payment-status/hold`,
      body: { ref: a.ref, reason: a.reason || null },
    }),
  },
];
